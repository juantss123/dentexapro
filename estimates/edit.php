...
if (!user_has_permission('estimates', $_SESSION['admin_permissions'] ?? '[]', $_SESSION['admin_role'] ?? 'employee')) {
    header('Location: ' . $path_to_root . 'dashboard.php?status=error&msg=' . urlencode('No tiene permiso para editar presupuestos.'));
    exit;
}

$estimate_id_to_edit = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
if (!$estimate_id_to_edit) {
    header('Location: index.php?status=error&msg=' . urlencode('ID de presupuesto no válido.'));
    exit;
}

$errors = [];
// Recuperar errores y datos del formulario de la sesión si existen (después de un error de guardado)
if (isset($_SESSION['estimate_form_errors_edit'])) { // Usar una clave de sesión diferente para edición
    $errors = $_SESSION['estimate_form_errors_edit'];
    unset($_SESSION['estimate_form_errors_edit']);
}
$form_data = $_SESSION['estimate_form_data_edit'] ?? []; // Usar una clave de sesión diferente
if (isset($_SESSION['estimate_form_data_edit'])) {
    unset($_SESSION['estimate_form_data_edit']);
}

// Cargar datos del presupuesto existente
$estimate_current_data = null;
$estimate_items_current = [];

$stmt_load_est = $mysqli->prepare("SELECT * FROM estimates WHERE id = ?");
if ($stmt_load_est) {
    $stmt_load_est->bind_param("i", $estimate_id_to_edit);
    $stmt_load_est->execute();
    $result_est = $stmt_load_est->get_result();
    $estimate_current_data = $result_est->fetch_assoc();
    $stmt_load_est->close();
}

if (!$estimate_current_data) {
    header('Location: index.php?status=error&msg=' . urlencode('Presupuesto no encontrado.'));
    exit;
}

$stmt_load_items = $mysqli->prepare("SELECT * FROM estimate_items WHERE estimate_id = ? ORDER BY id ASC");
if ($stmt_load_items) {
    $stmt_load_items->bind_param("i", $estimate_id_to_edit);
    $stmt_load_items->execute();
    $result_items = $stmt_load_items->get_result();
    while($item_row = $result_items->fetch_assoc()){
        $estimate_items_current[] = [
            'description' => $item_row['description'],
            'quantity' => $item_row['quantity'],
            'unit_price' => number_format($item_row['unit_price'], 2, ',', '.') // Formatear para mostrar con coma
        ];
    }
    $stmt_load_items->close();
}
if (empty($estimate_items_current)) { // Si por alguna razón no hay ítems, añadir uno vacío
    $estimate_items_current = [['description' => '', 'quantity' => 1, 'unit_price' => '']];
}


// Variables para repoblar el formulario: Priorizar datos de sesión (si hubo error), luego datos de BD
$patient_id_selected = $form_data['patient_id'] ?? $estimate_current_data['patient_id'];
$professional_name_val = trim($form_data['professional_name'] ?? ($estimate_current_data['professional_name_text'] ?: ($_SESSION['admin_name'] ?? 'Profesional no asignado')));
$estimate_date_val = $form_data['estimate_date'] ?? $estimate_current_data['estimate_date'];
$insurance_details_val = trim($form_data['insurance_details'] ?? $estimate_current_data['insurance_details']);
$notes_val = trim($form_data['notes'] ?? $estimate_current_data['notes']);
$status_val = $form_data['status'] ?? $estimate_current_data['status']; 
$estimate_number_display = $estimate_current_data['estimate_number']; // Para mostrar

$estimate_items_val = $estimate_items_current; // Por defecto, los de la BD
if (!empty($form_data['item_description']) && is_array($form_data['item_description'])) { // Si hay datos de sesión (error previo)
    $estimate_items_val = []; // Sobrescribir con datos del POST fallido
...
if(isset($_GET['status']) && $_GET['status'] === 'error_validation_save_edit' && isset($_SESSION['estimate_form_errors_edit'])){
    // $errors ya se cargó desde la sesión
} elseif (isset($_GET['status']) && $_GET['status'] === 'error_db_update' && isset($_GET['msg'])) {
    $errors[] = htmlspecialchars(urldecode($_GET['msg']));
} elseif (isset($_GET['status']) && $_GET['status'] === 'warning_email_failed_edit' && isset($_GET['msg'])) {
    $errors[] = "Advertencia: " . htmlspecialchars(urldecode($_GET['msg']));
}

$all_statuses = ['borrador', 'enviado', 'aprobado', 'rechazado', 'pagado'];
...
<head>
    <meta charset="UTF-8">
    <title>Editar Presupuesto N° <?php echo htmlspecialchars($estimate_number_display); ?> - DentexaPro</title>
...
HTML;
                ...
                <div class="card-body p-lg-4 p-3">
                    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                        <h2 class="mb-0 d-flex align-items-center">
                            <i class="fas fa-edit me-3 text-primary"></i>Editar Presupuesto N° <?php echo htmlspecialchars($estimate_number_display); ?>
                        </h2>
                    ...
                    <form action="actions/update_estimate.php" method="POST" id="estimateForm">
                        <input type="hidden" name="estimate_id" value="<?php echo $estimate_id_to_edit; ?>">
                        ...
                        <div id="estimateItemsContainer">
                            ...
                            <div class="row g-2 align-items-center item-row mb-2">
                                ...
                                <div class="col-md-1 col-2 d-flex align-items-end"> 
                                    <?php if ($index > 0 || count($estimate_items_val) > 1): // Mostrar siempre si hay más de 1, o si no es el primero ?>
                        ...
                        <hr class="my-4">
                        <h5 class="text-primary mb-3"><i class="fas fa-sticky-note me-2"></i>Notas y Estado</h5>
                         <div class="mb-3">
                        ...
                        <div class="mb-4">
                            <label for="status" class="form-label fw-bold">Estado del Presupuesto</label>
                            <select name="status" id="status" class="form-select form-select-lg">
                                <?php foreach($all_statuses as $stat): ?>
                                <option value="<?php echo $stat; ?>" <?php if($status_val == $stat) echo 'selected'; ?>><?php echo ucfirst($stat); ?></option>
                                <?php endforeach; ?>
                            </select>
                             <div class="form-text small">Seleccione el estado actual del presupuesto. Si lo cambia a "Enviado", se intentará (re)generar PDF y (re)enviar por email.</div>
                        </div>

                        <div class="d-flex justify-content-end gap-2">
                            <a href="index.php" class="btn btn-outline-secondary btn-lg"><i class="fas fa-times-circle me-1"></i>Cancelar</a>
                            <button type="submit" name="update_estimate_action" value="update_only" class="btn btn-info btn-lg"><i class="fas fa-save me-1"></i>Actualizar Presupuesto</button>
                            <button type="submit" name="update_estimate_action" value="update_send" class="btn btn-success btn-lg"><i class="fas fa-paper-plane me-1"></i>Actualizar y (Re)Enviar</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
    // El JavaScript es idéntico al de estimates/create.php
        ...
        function addRowEventListeners(row) {
            ...
            if (removeBtn) {
                removeBtn.addEventListener('click', function() {
                    // Permitir eliminar incluso si es la única fila en la página de edición
                    row.remove();
                    calculateTotal();
                    // Si no quedan filas, se podría añadir una vacía automáticamente, pero es opcional
                    if (itemsContainer.querySelectorAll('.item-row').length === 0) {
                        // Opcional: addItemBtn.click(); // Para añadir una fila vacía si se borran todas
                    }
                });
            }
        }
