...
if (!empty($search_term)) {
    ...
    $sql_search = "SELECT id, dni, fname, lname, birthdate FROM patients WHERE ";
    if ($search_by === 'dni') {
        $sql_search .= "dni LIKE ?";
    } elseif ($search_by === 'fname') {
        $sql_search .= "fname LIKE ?";
    } else { // Por defecto o si es 'lname'
        $sql_search .= "lname LIKE ?";
    }
    $sql_search .= " ORDER BY lname ASC, fname ASC";
    
    $stmt_patients = $mysqli->prepare($sql_search);
    if ($stmt_patients) {
...
<head>
    <meta charset="UTF-8">
    <title>Seleccionar Paciente para Ver Historial - Clínica Dental</title>
            ...
            <div class="card main-content-card shadow-sm rounded-3">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                        <h2 class="mb-0 d-flex align-items-center"><i class="fas fa-book-medical me-3 text-info"></i>Seleccionar Paciente para Ver Historial</h2>
                    ...
                    <form class="mb-4 p-3 border rounded bg-light" method="GET" action="history_select.php">
                    ...
                    <?php if ($search_performed): ?>
                                    ...
                                    <tbody>
                                        <?php while ($row = $patients_result->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="history.php?patient_id=<?php echo $row['id']; ?>" class="btn btn-info btn-sm">
                                                        <i class="fas fa-book-medical me-1"></i>Ver Historial
                                                    </a>
                                                </td>
                                            </tr>
                                        <?php endwhile; ?>
                        ...
                        <?php else: ?>
                            <div class="alert alert-warning mt-4">
                                <i class="fas fa-exclamation-triangle me-2"></i>No se encontraron pacientes con el término "<?php echo htmlspecialchars($search_term); ?>" buscando por <?php echo htmlspecialchars(ucfirst($search_by=="fname"?"nombre":($search_by=="lname"?"apellido":"DNI"))); ?>.
                            </div>
                        <?php endif; ?>
                    ...
                    <?php else: ?>
                        <div class="alert alert-light mt-4 text-center">
                            <i class="fas fa-search me-2"></i>Utilice el formulario de arriba para buscar un paciente y ver su historial clínico.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
